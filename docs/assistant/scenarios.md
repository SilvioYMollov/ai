...
function Pipeline() {
  ...
  async function run(subject: string) {
    ...
    const imageUrl = assistant.image.result?.images[0]?.url
    if (!imageUrl) return
    ...
    // 3. Pull the reply text back out and narrate it.
    const reply = assistant.chat.messages.at(-1)
    const caption = reply?.parts.find((part) => part.type === 'text')?.content
    if (!caption) return
...
**Why an assistant here:** the image URL feeds chat as [multimodal input](../media/image-generation#image-conditioned-generation), and the chat reply feeds speech — all typed, all on one connection. Step 2 doesn't have to know *where* the URL came from; it's just `assistant.image.result`. With three separate hooks you'd hand-carry each value across three independent clients and manage three connections. See [Text-to-Speech](../media/text-to-speech#playing-audio-in-the-browser) for turning `assistant.speech.result?.audio` into playable audio.
...
function ContentStudio() {
  ...
  async function publish(topic: string) {
    // (a) Draft the post as a typed object.
    await assistant.chat.sendMessage(`Write a short blog post about ${topic}.`)
    const draft = assistant.chat.final // { title: string; body: string } | null
    if (!draft) return
...
**Why an assistant here:** the draft's `title` becomes the image prompt and its `body` becomes the speech text — the handoffs are ordinary typed property reads (`draft.title`, `draft.body`), not values you serialized out of one hook and back into another. The entire workflow ships as a single endpoint (`/api/content`) with one auth boundary and one connection, which is exactly the shape a content-management feature wants. Because the `chat` callback declares `outputSchema`, you also get `assistant.chat.partial` for a live preview while the draft streams — see [Structured Outputs: Streaming UIs](../structured-outputs/streaming).
